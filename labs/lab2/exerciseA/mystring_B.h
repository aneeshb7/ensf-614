...
public:
  ...
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return 1 if lhs is greater than or equal to rhs. 

  bool operator <=(const Mystring& rhs) const; // less than or equal to operator
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return 1 if lhs is less than or equal to rhs.

  bool operator >(const Mystring& rhs) const; // greater than to operator
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return 1 if lhs is greater than to rhs. 

  bool operator <(const Mystring& rhs) const; // less than to operator
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return 1 if lhs is less than to rhs.

  bool operator ==(const Mystring& rhs) const; // equal to operator
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return 1 if lhs is equal to rhs.

  bool operator !=(const Mystring& rhs) const; // not equal to operator
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return 1 if lhs is not equal to rhs. 

  char& operator [](int pos) const; // index operator, modifiable
  // REQUIRES: pos is reference to the index within Mystring
  // PROMISES: returns char in that given position.

  friend ostream& operator<<(ostream& os, const Mystring& rhs); // insertion operator
  // REQUIRES: rhs is reference to a Mystring as a source
  // PROMISES: return the string of rhs. 
