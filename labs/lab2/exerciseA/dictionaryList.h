...
public:
  ...
  const char* operator [](int pos); // index operator, modifiable
  // REQUIRES: pos is reference to the index within DictionaryList
  // PROMISES: returns char* in that given position.

  friend ostream& operator<<(ostream& os, DictionaryList& rhs); // insertion operator
  // REQUIRES: rhs is reference to a DictionaryList as a source
